        ...
        Collections.sort(list, new Comparator<Integer[]>(){
            public int compare(Integer[] o1, Integer[] o2){
                if(o1[1].intValue() != o2[1].intValue()){
                    return o2[1].intValue()-o1[1].intValue(); // 점수 높은 순으로 내림차순
                }else{
                    return o1[0].intValue()-o2[0].intValue(); // 점수 같을 경우 수포자 오름차순
                }
            }
        });
        if(set.size() == 3){
            // 겹치지 않으므로 max 1개만 고르면 됨
            answer = new int[]{list.get(0)[0].intValue()};

        }else if(set.size() == 2){
            // 크기 비교해서 큰 것 1개 또는 2개 제출
            if(list.get(0)[1].intValue() == list.get(1)[1].intValue()){
                answer = new int[]{list.get(0)[0].intValue(), list.get(1)[0].intValue()};
            }else{
                answer = new int[]{list.get(0)[0].intValue()};
            }
